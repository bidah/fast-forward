...
function Provider(props) {
  ...
  const createCollection = useCallback(async () => {
    console.log({
      REACT_APP_ARTIST_CONTRACT_HOST_ACCOUNT:
        process.env.REACT_APP_ARTIST_CONTRACT_HOST_ACCOUNT,
    });
    ...
    const transactionId = await fcl
      .send([
        fcl.transaction`
          ...
          // TODO: [ok] Complete this transaction by calling LocalArtistMarket.withdraw().
          transaction(listingIndex: Int) {
            ...
            prepare(account: AuthAccount) {
              ...
              self.marketRef = baseAccount.getCapability<&{LocalArtistMarket.MarketInterface}>(/public/LocalArtistMarket).borrow() ?? panic("Couldn't borrow market capability")
            } 
            execute{
              self.marketRef.withdraw(listingIndex: listingIndex, to: self.userAccount.address)
            }
          }
        `,
