  ...
  describe("Fast Floward Tests", async () => {
    ...
    it(`removes a kittyitem from sale`, async () => {
      ...
      const collectionLength =
        await DappLib.kittyItemsMarketReadSaleCollectionLength(testData2);

      // 6) call kittyItemsMarketReadSaleCollectionIDs with testData3 and store the result
      const collectionIDs = await DappLib.kittyItemsMarketReadSaleCollectionIDs(
        testData3
      );

      // 7) assert the result from step 5) shows the SaleCollection has length 1
      assert.equal(collectionLength.result, 1, "collectionLength should be 1");

      // 8) assert the result (an array) from step 6) shows the SaleCollection has
      // an itemID == 1 in it at index 0
      assert.equal(
        collectionIDs.result[0],
        1,
        "SaleCollection first item has incorrect itemID"
      );
    });
    ...
    it(`checks both admin and user have correct balances after purchase`, async () => {
      ...
      const account1Balance = await DappLib.kibbleGetBalance(testData1);

      // 4) call kibbleGetBalance with testData2 and store the result
      const account2Balance = await DappLib.kibbleGetBalance(testData2);

      // 5) assert the result from step 3) equals 5.0
      assert(account1Balance, 5.0, "Balance does not match");

      // 6) assert the result from step 4) equals 25.0
      assert(account2Balance, 25.0, "Balance does not match");
    });
  });
});
