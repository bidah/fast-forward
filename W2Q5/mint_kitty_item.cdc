...
transaction(recipient: Address, typeID: UInt64) {
    ...
    prepare(signer: AuthAccount) {
      ...
      self.minter = signer.borrow<&KittyItems.NFTMinter>(from: KittyItems.MinterStoragePath) ?? panic("Couldn't borrow a reference to the NFTMinter resource in the signer's storage")
      ...
      // 2) borrow a public reference to the recipient's Kitty Items Collection
       self.receiver = getAccount(recipient)
            ...
            .borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Couldn't borrow a public reference to the recipient's Kitty Items Collection")
    }
